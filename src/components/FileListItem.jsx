...
export default function FileListItem({ file, onOpen, onCopy, isCopied }) {
  ...
  return (
    <li className="card flex flex-col gap-3 rounded-xl p-4 transition hover:shadow-md sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-1 items-start gap-3">
        <div className="mt-1 flex h-10 w-10 items-center justify-center rounded-lg bg-surface-muted" aria-hidden="true">
          {icon}
        </div>
        <div className="min-w-0">
          <p className="truncate text-base font-semibold text-primary" title={title}>
            {title}
          </p>
          {sizeLabel && (
            <span className="mt-2 inline-flex items-center rounded-full bg-surface-muted px-2.5 py-0.5 text-xs font-medium text-textc-muted">
              {sizeLabel}
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="btn-primary inline-flex items-center justify-center px-4 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-60"
        >
          Open
        </button>
        <button
